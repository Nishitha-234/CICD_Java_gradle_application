            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 13.233.156.192:8083/springapp:${VERSION} .
                                docker login -u admin -p $docker_password 13.233.156.192:8083 
                                docker push  13.233.156.192:8083/springapp:${VERSION}
                                docker rmi 13.233.156.192:8083/springapp:${VERSION}
                            '''
                    }
                }
            }
        ...
        stage('indentifying misconfigs using datree in helm charts'){
            steps{
                script{
                    ...
                    dir('kubernetes/') {
                        withEnv(['DATREE_TOKEN=x4p3wdfS7stnHuGvPcGMeg']) {
                    ...
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                          dir('kubernetes/') {
                             sh '''
                                 ...
                                 curl -u admin:$docker_password http://13.233.156.192:8081/repository/helm-hosted/ --upload-file myapp-${helmversion}.tgz -v
                            '''
                          }
                    }
                }
